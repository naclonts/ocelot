...
def _make_face(size: int = 512) -> np.ndarray:
    ...
    cv2.ellipse(img, (cx, cy), (int(size * 0.34), int(size * 0.42)), 0, 0, 360, face_colour, -1)
    ...
    for side in (-1, 1):
        cv2.ellipse(
            img, (cx + side * eye_sep, brow_y), (brow_w, brow_h), 0, 0, 360, brow_colour, -1
        )
        # Blend brow edge back toward skin for natural look
        cv2.ellipse(
            img,
            (cx + side * eye_sep, brow_y + brow_h),
            (brow_w, brow_h // 2),
            0,
            0,
            360,
    ...
    for side in (-1, 1):
        ex = cx + side * eye_sep
        cv2.ellipse(img, (ex, eye_y), (sclera_rx, sclera_ry), 0, 0, 360, sclera_colour, -1)
    ...
    cv2.ellipse(img, (cx, nose_bot_y), (nose_w, int(size * 0.03)), 0, 0, 360, nose_colour, -1)
    ...
    cv2.ellipse(
        img,
        (cx, mouth_y + mouth_ry),
        (mouth_rx, mouth_ry + int(size * 0.02)),
        0,
        0,
        180,
        shadow_colour,
    ...
    # Upper lip line
    cv2.ellipse(img, (cx, mouth_y), (mouth_rx, mouth_ry), 0, 0, 180, lip_colour, 4)

    # ── Chin ──────────────────────────────────────────────────────────
    chin_y = int(cy + size * 0.38)
    cv2.ellipse(
        img, (cx, chin_y), (int(size * 0.18), int(size * 0.04)), 0, 0, 360, (145, 108, 78), -1
    )
...
def main() -> None:
    ...
    out_dir = os.path.join(repo_root, "sim", "models", "face_billboard", "materials", "textures")
    ...
    try:
        ...
        if cascade_path:
            ...
            low_res_result = "DETECTED ✓" if detected else "not detected (expected at low res)"
            print(f"Haar self-check at 76×76: {low_res_result}")
            ...
            full_res_result = "DETECTED ✓" if len(faces_full) > 0 else "not detected"
            print(f"Haar self-check at 512×512: {full_res_result}")
        else:
