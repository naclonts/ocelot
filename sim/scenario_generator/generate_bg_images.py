...
REAL_PHOTOS = [
    ...
    {
        "id": "indoor_bedroom_cluttered",
        "tags": ["indoor", "busy"],
        "file": "indoor_bedroom_cluttered.jpg",
    },
    {
        "id": "indoor_workshop_cluttered",
        "tags": ["indoor", "busy"],
        "file": "indoor_workshop_cluttered.jpg",
    },
]
...
def _rebuild_manifest(generated_ids: set[str]) -> None:
    ...
    real_entries = [e for e in REAL_PHOTOS if (_BACKGROUNDS_DIR / e["file"]).exists()]
...
def main():
    ...
    parser.add_argument(
        "--start", type=int, default=0, help="0-based index of first scene to process (default: 0)."
    )
    parser.add_argument(
        "--limit",
        "-n",
    ...
    parser.add_argument(
        "--delay", type=float, default=1.0, help="Seconds to wait between API calls (default: 1.0)."
    )
    ...
    print(
        ...
        f"quality=high  size=1536×1024  delay={args.delay}s"
    )
    print()
